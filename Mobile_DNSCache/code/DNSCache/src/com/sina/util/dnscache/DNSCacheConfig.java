...
public class DNSCacheConfig {
     ...
     * 配置接口api
     */
    private static final String configApi = "http://api.weibo.cn/2/httpdns/config";

    public static ArrayList<String> domainSupportList = new ArrayList<String>();

    public static void setDafaultCfg(Context ctx, String json) {
        ...
        if (ENABLE_UPDATE_CONFIG){
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        /*
                         * 调用太快，AppConfigUtil.getAppKey()可能会取到null
                         */
                        Thread.sleep(500);
                    } catch (InterruptedException e1) {
                        e1.printStackTrace();
                    }
                    INetworkRequests netWork = new HttpClientNetworkRequests();
                    try {
                        String api = configApi;
                        HashMap<String, String> params = new HashMap<String, String>();
                        params.put("k", AppConfigUtil.getAppKey());
                        params.put("v", AppConfigUtil.getVersionName());
                        params.putAll(AuthCenter.getAuthParmas());
                        HttpResult httpResult = netWork.request(api, null, params, (InputStream)null, RequestType.GET);
                        if (AuthValidator.check(httpResult.responseStr)) {
                            Data data = Data.fromJson(httpResult.responseStr);
                            if (null != data) {
                                saveLocalConfigAndSync(ctx, data);
                            }
                            HttpDnsLogManager.getInstance().writeLog(HttpDnsLogManager.TYPE_INFO, HttpDnsLogManager.ACTION_INFO_CONFIG, httpResult.responseStr);
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                        HttpDnsLogManager.getInstance().writeLog(HttpDnsLogManager.TYPE_ERROR, HttpDnsLogManager.ACTION_INFO_CONFIG, "{\"errMsg\":" + "\"" + e.getMessage() + "\"}");
                    }
                }
            }).start();
        }
    ...
    public static class Data {
        ...
        public static Data createDefault() {
            ...
            model.HTTPDNS_SERVER_API.add("http://dns.weibo.cn");
            model.HTTPDNS_SERVER_API.add("http://202.108.7.232/");
            model.HTTPDNS_SERVER_API.add("http://221.179.190.246/");
            model.HTTPDNS_SERVER_API.add("http://58.63.236.228/");
            ...
            model.DOMAIN_SUPPORT_LIST.add("ww1.sinaimg.cn");
            model.DOMAIN_SUPPORT_LIST.add("ww2.sinaimg.cn");
            model.DOMAIN_SUPPORT_LIST.add("ww3.sinaimg.cn");
            model.DOMAIN_SUPPORT_LIST.add("ww4.sinaimg.cn");
            model.DOMAIN_SUPPORT_LIST.add("api.weibo.cn");
            return model;
        }
