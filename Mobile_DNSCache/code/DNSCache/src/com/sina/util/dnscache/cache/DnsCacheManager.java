...
public class DnsCacheManager extends DNSCacheDatabaseHelper implements IDnsCache {
     ...
     */
    @SuppressLint("NewApi")
    private ConcurrentHashMap<String, DomainModel> data = new ConcurrentHashMap<String, DomainModel>();
     ...
     * 删除内存中数据
     */
    public DomainModel removeMemoryCache(String key) {
    ...
    public void addMemoryCache(String url, DomainModel model) {
        ...
        if (model == null)
            return;
        if (model.ipModelArr == null)
            return;
        if (model.ipModelArr.size() <= 0)
            return;
        ...
        Tools.log("TAG", "插入内存::" + model.domain);
        data.put(url, model);
    }
